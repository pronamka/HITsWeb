...
function generateMaze(){
  ...
  let sX = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(x => isEven(x)));
  let sY = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(y => isEven(y)));
  setTile(sX, sY, 0); 
  ...
  startX = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(x => isEven(x)));
  startY = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(y => isEven(y)));
  setTile(startX, startY, 2); 

  finishX;
  finishY;
  do {
    finishX = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(x => isEven(x)));
    finishY = getRandomFrom(Array.from({ length: size}, (_, index) => index).filter(y => isEven(y)));
...
const createArray = (size, initial) =>
  Array.from({ length: size * size }, (_, i) => initial(i));
...
class aStar {
  ...
  getNeighbours(node) {
      ...
      for (let dir of directions) {
          ...
          if (nx >= 0 && nx < this.grid[0].length && ny >= 0 &&
              ny < this.grid.length && this.isOpen(nx, ny)) {
          ...
          for (let neighbour of neighbours) {
            if (isStopped) return [];
              ...
              if (!inOpenList) {
                  ...
                  let existingNode = this.getNodeFromList(this.openList, neighbour.x, neighbour.y);

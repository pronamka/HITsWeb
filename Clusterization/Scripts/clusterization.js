...
function kMeansClustering(points, k, maxIterations = 100, convergenceThreshold = 0.001) {
    if (points.length < k) {
        return Array(points.length).fill().map((_, i) => i);
    }
    ...
    while (iterations < maxIterations) {
        ...
        let newCentroids = Array(k).fill().map(() => ({ y: 0, x: 0, count: 0 }));
        ...
        for (let i = 0; i < k; i++) {
            if (newCentroids[i].count > 0) {
                ...
                const shift = Math.sqrt(
                    Math.pow(newY - centroids[i].y, 2) +
                    Math.pow(newX - centroids[i].x, 2)
                );
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        totalDistance += Math.sqrt(
            Math.pow(points[i].y - centroid.y, 2) +
            Math.pow(points[i].x - centroid.x, 2)
        );
    }
...
function cancelAllAlg(){
    ...
    pointsArray.forEach((cell) => {
        const cellElement = document.getElementById(`cell-${cell.y}-${cell.x}`);
        for(let i = 0; i < 3; i++){
...
function applyKMeansColoring(){
    ...
    pointsArray.forEach((cell, index) => {
        ...
        if (screenState.secondAlg === true && screenState.thirdAlg === true) {
            ...
            applyHierarchicalColoring();
        } else if ((screenState.secondAlg === false && screenState.thirdAlg === true) || (screenState.secondAlg === true && screenState.thirdAlg === false)) {
            cellElement.children[0].style.backgroundColor = algorithmColors[0][group];
...
function applyMiniBatchKMeansColoring(){
    ...
    pointsArray.forEach((cell, index) => {
        ...
        if (screenState.secondAlg === true && screenState.firstAlg === true) {
            ...
            applyHierarchicalColoring();
        } else if ((screenState.secondAlg === false && screenState.firstAlg === true) || (screenState.secondAlg === true && screenState.firstAlg === false)) {
            cellElement.children[2].style.backgroundColor = algorithmColors[2][group];
...
function initGrid() {
    ...
    grid.innerHTML = '';
    cells = Array(WIDTH).fill().map(() => Array(HEIGHT));
        ...
        for(let j = 0; j < HEIGHT; j++) {
            ...
            cell.addEventListener("click", function() {
                if(chosenCells.has(cells[i][j])) {
                    ...
                    document.getElementById(`cell-${i}-${j}`).style.backgroundColor = "#b5b094";
            ...
            for(let i = 0; i < 3; i++){
                ...
                if(i === 1){
                    for(let j = 0; j < 2; j++) {
                        ...
                        subSubCell.className = "sub-sub-cell";
